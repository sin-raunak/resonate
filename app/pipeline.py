...
def run_pipeline(manifest_rows: list[ManifestRow]) -> list[ClipQCReport]:
    ...
    for row in manifest_rows:
        ...
        exact_dup_result = check_exact_duplicates(
            row.filepath, row.clip_id, seen_hashes
        )
        ...
        near_dup_result = check_near_duplicates(
            row.filepath, row.clip_id, seen_fingerprints
        )
        ...
        text_match_result = check_text_audio_match(
            row.filepath, row.proposed_text, all_texts
        )
